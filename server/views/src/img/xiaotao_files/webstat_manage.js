...
(function(){
	...
	if(window.location.host.indexOf(".baike.com")>0){
		hdClickS.src = 'http://www.huimg.cn/stat/js/XBKClickMonitor.js';
		_isStatAdClick = true;
...
function _hdCO() {
	...
	if(window.location.host.indexOf(".hudong.com")>0){
		...
		sc.src = "http://stat.baike.com/hdWebXBKStat.do?random=" + Math.random() + "&hd_accessrefer=" + encodeURIComponent(_hd_refer) + "&hd_accessurl=" + encodeURIComponent(hd_accessurl) + "&hd_res=" + hd_res;
	}
...
function StatVirtualTraffic(hd_accessrefer, hd_accessurl, hd_virtual) {
	...
	if(window.location.host.indexOf(".hudong.com")>0){
		...
		sc.src = 'http://stat.baike.com/hdWebXBKStatVirtual.do?random=' + Math.random()+'&hd_virtual='+hd_virtual+'&hd_accessrefer='+encodeURIComponent(hd_accessrefer)+'&hd_accessurl='+encodeURIComponent(hd_accessurl)+"&hd_res="+ hd_res;
		_isStatVirtual = true;
	}
	if(_isStatVirtual){
		document.getElementsByTagName("head")[0].appendChild(sc);
